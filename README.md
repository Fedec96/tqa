...
| lookup.results   | Lookup\<TResponse\>                                       | The key to the response's field that represents the page's record(s). Must lead to an array. It can be a function for conditional return                                                 |
| lookup.total     | Lookup\<TResponse\> \| LookupCallback\<TResponse, Total\> | The key to the response's field that represents the total result(s). Must lead to a number or a string containing a number. It can be a function for conditional return or nested values |
